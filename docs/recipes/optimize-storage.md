...
rrweb provides an fflate-based simple compress function in [@supademo/rrweb-packer](../../packages/packer/).

You can use it by passing it as the `packFn` in the recording.

```js
import { pack } from '@supademo/rrweb-packer';
...
```js
import { unpack } from '@supademo/rrweb-packer';
