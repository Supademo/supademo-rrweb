...
rrweb 代码分为录制和回放两部分，大多数时候用户在被录制的应用中只需要引入录制部分代码。同样可以通过使用 @supademo/rrweb-record 包和 CDN 服务来实现：
...
除了 `rrweb` 和 `@supademo/rrweb-record` 包之外，rrweb 还提供了其他不同用途的包。
...
- [@supademo/rrweb-all](packages/all)：一个包含 `rrweb` 和 `@supademo/rrweb-packer`，便于安装的包。
- [@supademo/rrweb-record](packages/record)：一个用于录制 rrweb 会话的包。
- [@supademo/rrweb-replay](packages/replay)：一个用于回放 rrweb 会话的包。
- [@supademo/rrweb-packer](packages/packer)：一个用于打包和解包 rrweb 数据的包。
- [@supademo/rrweb-types](packages/types)：包含 rrweb 包中共享的类型定义。
- [@supademo/rrweb-utils](packages/utils)：包含 rrweb 包中共享的工具函数。
